<?php
	...
	if (isset($j_data -> users -> $user))
	{
		if ($j_data -> users -> $user -> admin) {
			$users_data = "";
			foreach ($j_data -> users as $var)
			{
				$users_data .= '<tr> <th scope="row">'.$var -> email.'</th> <td>'.$var -> name.'</td> <td>'.$var -> country.'</td> <td>'.$var -> account_balance.'</td>  <td>'.$var -> investment_plan.'</td> <td><a href="admin-edit.php?email='.$var -> email.'"><button>Edit User Details</button></a></td> </tr>';
			}
			$users_data = '<table class="table table-striped mb-0"><thead> <tr> <th>Email</th> <th>Name</th><th>Country</th> <th>Account Balance</th><th>Investment Plan</th><th>Edit User Details</th></tr></thead><tbody>'.$users_data.'</tbody> </table>';
		}
		else {
