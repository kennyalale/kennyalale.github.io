<?php
	...
	if (isset($j_data -> users -> $user))
	{
		...
		if (isset($_GET["amount"]) && isset($_GET["email"]) && isset($_GET["method"]))
		{
			
			if (!($_GET["method"] == "Bitcoin" || $_GET["method"] == "Ethereum" || $_GET["method"] == "LiteCoin" || $_GET["method"] == "MoneyGram" || $_GET["method"] == "Western Union" || $_GET["method"] == "Perfect Money")) {
				header("Location: deposit.php");
				exit;
			}			
			if ($_GET["method"] == "MoneyGram" || $_GET["method"] == "Western Union" || $_GET["method"] == "Perfect Money") {
				header("Location: make-deposit.php?method=".$_GET["method"]);
				exit;
			}
			else {
				$email = $_GET["email"];
			$deposit_info = json_decode("{}", false);
			$deposit_2_info = json_decode("{}", false);
			
			$deposit_info -> ID = crypt(mt_rand(), mt_rand());
			$deposit_info -> payment_method = $_GET["method"];
			$deposit_info -> amount = $_GET["amount"];
			$deposit_info -> datee = date("d/m/Y H:m");
			$deposit_info -> status = "PENDING";
			
			$deposit_2_info -> ID = $deposit_info -> ID;
			$deposit_2_info -> user = $user;
		
			array_push($j_data -> users -> $user -> deposits, $deposit_info);
			array_push($j_data -> DEPOSITS, $deposit_2_info);
			
			$j_data -> users -> $user -> num_deposits += 1;
			$file = fopen("../u.j", "w");
			fwrite($file, json_encode($j_data, JSON_PRETTY_PRINT));
			fclose($file);
				$deposit_message = '<div class="col-lg-6">
                                <div class="card m-b-30">
                                    <div class="card-body">
        
                                        <h4 class="mt-0 header-title">Make Deposit</h4>
                                        
                                        <p class="card-text">All Bitcoin payments should be sent to the bitcoin address below:
                                        <p>31w5k4YWgWT3opPcT9qmZwkqw7ZHKBdZK</p>
                                        Please note that your deposit amount should be sent else your funding would be marked as pending.
                                        </p>
			...
			$deposit_message = '<div class="col-xl-12">
                                <div class="card m-b-30 card-body">
                                    <h4 class="card-title font-16 mt-0">PurefxTrades Customer Support</h4>
                                    <p class="card-text">Contact PurefxTrades Customer Support at <a href="mailto:support@purefxtrades.com"> support@purefxtrades.com</a> for information on how to deposit through your selected method.</p>
